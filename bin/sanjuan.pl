...
sub print_help{
	...
	print "\t-nprocs 1 -g hg19 -g1 COND -g2 CNTR -d . -b M -o . -p phred33 -l 2S -a AGATCGGAAGAGC -c HC -min1read Y\n";
...
if($start_with ne "S"){
	...
	if(!is_available_for_mapping($genome,$abs_path)){die "Mapping for species / genome $genome is not possible\nas this genome is not installed for mapping.\nRun sanjuan -showg to see installed genomes,\nand see README on GitHub.com webpage of SANJUAN on how to install further genomes\n";}
}else{
	# user wants to do splicing analysis only
	if(!is_available_for_splicing($genome,$sanjuan_genomic_data_dir)){die "Splicing analysis for genome $genome is not possible \nas this genome is not installed for splicing analysis.\nYou are using db directory $sanjuan_genomic_data_dir\nRun sanjuan -showg to see installed genomes, \nsee README on GitHub.com webpage of SANJUAN on how to install further genomes\n";}
}
